...
function loadPackageData(root = process.cwd()) {
    ...
    if (fs.existsSync(pkg)) {
        const _require = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunks/lib-D9_OPmIh.cjs', document.baseURI).href)));
...
const _require$1 = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunks/lib-D9_OPmIh.cjs', document.baseURI).href)));
    ...
    return [
        {
            ...
            config(config) {
                ...
                config.build.ssr = true;
                config.build.ssrEmitAssets = true;
                config.ssr = vite.mergeConfig(defaultConfig.ssr, config.ssr || {});
